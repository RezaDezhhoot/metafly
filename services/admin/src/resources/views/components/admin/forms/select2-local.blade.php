@props(['id', 'label','hidden' => false,'data' => null,'width' => 12, 'value' => false  ,'multiple' => false ,'current' => null, 'key' => null , 'text' => null,'required' => false])
        ...
        <div class=" d-flex align-items-center" wire:ignore >
            <select  id="{{$id}}"
                ...
                @foreach($data as $i => $item)
                    ...
                    @else
                        <option {{ $current == ($key ? ($item[$key] ?? null) : $i) ? 'selected' : '' }} value="{{ $key ? ($item[$key] ?? null) : $i }}">{{$text ? ($item[$text] ?? '') : $item }}</option>
                    @endif
                @endforeach
            </select>
        </div>
